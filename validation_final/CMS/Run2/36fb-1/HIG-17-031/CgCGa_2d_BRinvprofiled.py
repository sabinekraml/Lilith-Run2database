...
def usrXMLinput(mass=125.09, CGa=1, Cg=1, BRinv=0, precision="BEST-QCD"):
...
"""
    myInput = {'mass': mass, 'CGa': CGa, 'Cg': Cg, 'BRinv': BRinv, 'precision': precision}
...
def getL(CGa, Cg, BRinv):
    myXML_user_input = usrXMLinput(mass=myhmass, CGa=CGa, Cg=Cg, BRinv=BRinv, precision=myprecision)
...
for CGa in np.linspace(CGa_min, CGa_max, grid_subdivisions):
    ...
    for Cg in np.linspace(Cg_min, Cg_max, grid_subdivisions):
        ...
        #lilithcalc.computelikelihood(userinput=myXML_user_input)
        #m2logL = lilithcalc.l
        m = Minuit(getL, CGa=CGa, fix_CGa=True, Cg=Cg, fix_Cg=True, BRinv=0.2, limit_BRinv=(0,0.99), print_level=0, errordef=1, error_CGa=0.5, error_Cg=0.5, error_BRinv=0.05)
        m.migrad()
        m2logL = m.fval
...
plt.text(0.75, 1.5, r'Validation of', fontsize=13)
plt.text(0.75, 1.45, r'CMS-HIG-17-031', fontsize=13)
plt.text(0.75, 1.4, r'(BRinv profiled)', fontsize=13)
...
fig.savefig("validation/CMS/HIG-17-031-CgCGa-BRinvprofiled.pdf")
