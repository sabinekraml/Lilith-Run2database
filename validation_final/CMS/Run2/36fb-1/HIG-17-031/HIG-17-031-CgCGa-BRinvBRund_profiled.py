...
for CGa in np.linspace(CGa_min, CGa_max, grid_subdivisions):
    ...
    for Cg in np.linspace(Cg_min, Cg_max, grid_subdivisions):
        ...
        #lilithcalc.computelikelihood(userinput=myXML_user_input)
        #m2logL = lilithcalc.l
        m = Minuit(getL, CGa=CGa, fix_CGa=True, Cg=Cg, fix_Cg=True, BRinv=0.1, limit_BRinv=(0,0.48), BRund=0.1, limit_BRund=(0,0.48), print_level=0, errordef=1, error_BRinv=0.05, error_BRund=0.05)
        m.migrad()
        m2logL = m.fval
...
plt.text(0.75, 1.455, '(BR_inv, BR_und profiled)', fontsize=13)
...
fig.savefig("validation/CMS/HIG-17-031-CgCGa_BRinvBRund_profiled.pdf")
