...
m = Minuit(getL, CGa=1, limit_CGa=(0,5), Cg=1, limit_Cg=(0,5), BRinv=0.2, limit_BRinv=(0,1), errordef=1, error_CGa=0.5, error_Cg=0.5, error_BRinv=0.1)
...
# Profiling over CGa
xGa,yGa,rGa = m.mnprofile('CGa', bins=300, bound=(0., 3), subtract_min=True)

# Profiling over Ca
xg,yg,rg = m.mnprofile('Cg', bins=300, bound=(0., 3), subtract_min=True)

# Profiling over extraBR
xBR,yBR,rBR = m.mnprofile('BRinv', bins=300, bound=(0., 0.8), subtract_min=True)
...
plt.ylim([0,8])
plt.xlim([0.,0.5])
plt.xlabel(r'BR(invisible or undetected)', fontsize=28)
plt.ylabel(r'$-2\,\log\,L/L_0$', fontsize=28)
plt.axhline(y=1.,color='k',ls='dashed')
plt.axhline(y=4.,color='k',ls='dashed')
plt.axhline(y=9.,color='k',ls='dashed')
...
fig.savefig("validation/CMS/HIG-17-031-CgCGaBRinv-1dprofiles.pdf")
