...
class ReadExpInput:
    ...
    def read_file(self, filepath):
        ...
        (bestfit, alpha, nu, param, grid, Lxy, LChi2min) = self.read_mus(root, dim, type)

        self.mu.append({"filepath": self.filepath,
                        "dim": dim, "type": type,
                        "eff": eff,
                        "bestfit": bestfit, "param": param, "grid": grid, "alpha": alpha, "nu": nu,
    ...
    def read_mus(self, root, dim, type):
        ...
        # first, read the alpha
        alpha = {}
        for child in root:
            if child.tag == "alpha":
                alpha = float(child.text)
        ...
        return (bestfit, alpha, nu, param, grid, Lxy, LChi2min)
