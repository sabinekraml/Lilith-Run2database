...
# f = open('HIGG_2016-21_Llh-2d-Grid68.txt', 'r')
f = open('HIGG_2017-14_Llh-2d-Grid95.txt', 'r')
...
def func(X, sig1p, sig1m, sig2p, sig2m, p):
    z1, z2 = X[:,0], X[:,1]
    z10, z20 = 2.857142857,	2.28668942
...
expdata = np.genfromtxt('HIGG_2017-14_Llh-2d-GridFull.txt')
xExp = expdata[:,0]
yExp = expdata[:,1]
...
plt.xlabel(r'$\mu_{WW}^{ZH}$', fontsize=20)
plt.ylabel(r'$\mu_{WW}^{WH}$', fontsize=20)
plt.title("$\mu$ from ATLAS-HIGG-2017-14 (VGaussian, 95% CL, fitted corr)")
...
fig.savefig('HIGG-2017-14-mu-VGaussian-fit2d-VGaussian-95CL.pdf')
