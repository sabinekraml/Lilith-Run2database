...
xmax, ymax = 3.3, 3.3
x = np.arange(-0.5, xmax, 0.01, dtype=np.float)
y = np.arange(-0.5, ymax, 0.01, dtype=np.float)
X, Y = np.meshgrid(x, y)
Z = Loglikelihood(Y,X)
...
expdata = np.genfromtxt('HIGG-2017-07_Llh-2d-GridFull.txt')
xExp = expdata[:,0]
yExp = expdata[:,1]
...
plt.title("$\mu$ ATLAS-HIGG-2017-07 (Poisson, 68% CL, fitted corr)")
# plt.title("$\mu$ ATLAS-HIGG-2016-22 (Poisson, 95% CL, fitted corr)")
...
# fig.savefig('mu_ggH-VBF_2D_Poisson-fitted-corr-95CL.pdf')
# fig.savefig('mu_ggH-VBF_2D_Poisson-fitted-corr-68CL.pdf')
